...
class NNBase(object):
    ...
    def __init__(self, param_dims_dense,
        ...
        #print "the sparams is : ",self.sparams
        self.sgrads = SparseDeltas(**param_dims_sparse)
        #print "the sgrads is : ",self.sgrads 
        #print self.sgrads
    ...
    def _apply_grad_acc(self, alpha=1.0):
        ...
        """
        # Dense updates
        if(np.sqrt(np.sum(self.grads.full**2)) > 5):
            self.params.full -= alpha * (5 / np.sqrt(np.sum(self.grads.full**2))) * self.grads.full
        else:
    ...
    def predict(self, xs,ys,pred):
        ...
        #    for t in xrange(ns):
        #        hs[t] = tanh(dot(self.params.H, hs[t - 1]) + self.sparams.L[y] * x[t])
        #        ps[t] = softmax(dot(self.params.U, hs[t]))
        #    pred[idx] = np.argmax(ps[ns-1])
        #    idx += 1
        #return pred
