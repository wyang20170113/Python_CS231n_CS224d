        ...
        if (node.fprop == False):
            if (node.isLeaf == True):
                ...
                if (node.left != None and node.left.fprop == True and node.right != None and node.right.fprop == True):
                    ...
                    node.hActs1 = np.maximum(self.W.dot(h_pre) + self.b,0)
        ...
        if (node.isLeaf == False):
             if(node.left != None or node.right != None):
                ...
                if(node.left != None):
                    ...
                    self.backProp(node.left,self.W[:,0:self.wvecDim].T.dot(error_down))
            
                if(node.right != None):
                    self.dW[:,self.wvecDim:] += np.outer(error_down,node.right.hActs1)
                    self.backProp(node.right,self.W[:,self.wvecDim:].T.dot(error_down))
        else:
            mask = np.zeros(self.outputDim,)
            mask[node.label] = 1.0
            delta = node.probs - mask
